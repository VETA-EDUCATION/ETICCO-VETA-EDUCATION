...
class ViewConvertLead extends SugarView
{
    ...
    public function display() {
        ...
        //$smarty->assign( "fecha_viaje_2_c" , $this->focus->fecha_viaje_2_c );
        global $mod_strings;
    ...
    protected function handleSave() {
        ...
        foreach( $beans as $bean ) {
            if( ! empty( $lead ) ) {
                ...
                if( ! empty( $leadsRel ) ) {
                    ...
                    /* BEGIN - SECURITY GROUPS */ global $sugar_config;
            ...
            /*if(isset($_REQUEST['fecha_de_viaje_c']))
                $bean->fecha_de_viaje_c = $_REQUEST['fecha_de_viaje_c'];*/
        ...
        if( ! empty( $lead ) ) {    //Mark the original Lead converted
            ...
            if( ! empty( $lead->id ) && ! empty( $lead->photo ) && ! empty( $beans[ 'Contacts' ]->id ) && ! empty( $beans[ 'Contacts' ]->photo ) ) {
                $bCopied = false;
                if( ( $lead->photo === $beans[ 'Contacts' ]->photo ) && is_readable( 'upload/' . $lead->id . '_photo' ) ) {
                    $bCopied = copy( 'upload/' . $lead->id . '_photo' , 'upload/' . $beans[ 'Contacts' ]->id . '_photo' );
                }
                if( $bCopied ) {
                    ...
                    LoggerManager::getLogger()->warning( 'Lead conversion: failed copying upload/' . $lead->id . '_photo to upload/' . $beans[ 'Contacts' ]->id . '_photo' );
                }
            }

            // Agregado por SOEL para crear relaciones entre contacto y visas, informacion academica, pasaportes, otros nombres, ciudadania, servicios adicionales, solvencia economica, informacion laboral
            if( $bean->object_name == "Contact" ) {
                // Copiando las relaciones del modulo de informacion academica

                $ias = $lead->get_linked_beans( 'veta_informacionacademica_leads' , 'Veta_InformacionAcademica' );

                foreach( $ias as $ia ) {
                    $bean->load_relationship( 'veta_informacionacademica_contacts' );
                    $bean->veta_informacionacademica_contacts->add( $ia->id );
                }

                // Copiando las relaciones del modulo de visas

                $visas = $lead->get_linked_beans( 'veta_visas_leads' , 'Veta_Visas' );

                foreach( $visas as $visa ) {
                    $bean->load_relationship( 'veta_visas_contacts' );
                    $bean->veta_visas_contacts->add( $visa->id );
                }

                // Copiando las relaciones de pasaportes

                $pasaportes = $lead->get_linked_beans( 'veta_pasaporte_leads' , 'Veta_Pasaporte' );

                foreach( $pasaportes as $pasaporte ) {
                    $bean->load_relationship( 'veta_pasaporte_contacts' );
                    $bean->veta_pasaporte_contacts->add( $pasaporte->id );
                }

                // Copiando las relaciones de otros nombres

                $ons = $lead->get_linked_beans( 'veta_otrosnombres_leads' , 'Veta_OtrosNombres' );

                foreach( $ons as $on ) {
                    $bean->load_relationship( 'veta_otrosnombres_contacts' );
                    $bean->veta_otrosnombres_contacts->add( $on->id );
                }

                // Copiando las relaciones de ciudadania
                $ciudadanias = $lead->get_linked_beans( 'veta_ciudadania_leads' , 'Veta_Ciudadania' );

                foreach( $ciudadanias as $ciudadania ) {
                    $bean->load_relationship( 'veta_ciudadania_contacts' );
                    $bean->veta_ciudadania_contacts->add( $ciudadania->id );
                }

                // Copiando las relaciones de servicios adicionales
                $servicios = $lead->get_linked_beans( 'veta_serviciosadicionales_leads' , 'Veta_ServiciosAdicionales' );

                foreach( $servicios as $servicio ) {
                    $bean->load_relationship( 'veta_serviciosadicionales_contacts' );
                    $bean->veta_serviciosadicionales_contacts->add( $servicio->id );
                }

                // Copiando las relaciones de solvencia economica
                $solvencias = $lead->get_linked_beans( 'veta_solvenciaeconomica_leads' , 'Veta_SolvenciaEconomica' );

                foreach( $solvencias as $solvencia ) {
                    $bean->load_relationship( 'veta_solvenciaeconomica_contacts' );
                    $bean->veta_solvenciaeconomica_contacts->add( $solvencia->id );
                }

                // Copiando las relaciones de informacion laboral
                $ilaborales = $lead->get_linked_beans( 'veta_informacionlaboral_leads' , 'Veta_InformacionLaboral' );

                foreach( $ilaborales as $ilaboral ) {
                    $bean->load_relationship( 'veta_informacionlaboral_contacts' );
                    $bean->veta_informacionlaboral_contacts->add( $ilaboral->id );
                }
            }
        }
     ...
     */
    protected function populateNewBean( $module , $bean , $contact , $lead ) {
    ...
    protected function findRelationship( $from , $to ) {
        ...
        foreach( $from->field_defs as $field => $def ) {
            if( isset( $def[ 'type' ] ) && $def[ 'type' ] == "link" && isset( $def[ 'relationship' ] ) ) {
                ...
                elseif( isset( $dictionary[ $rel_name ] ) && isset( $dictionary[ $rel_name ][ 'relationships' ] ) && isset( $dictionary[ $rel_name ][ 'relationships' ][ $rel_name ] ) ) {
