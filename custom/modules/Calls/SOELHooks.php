...
class SOELHooks
{
    ...
    public function relacionar_estudiante( &$bean , $event , $arguments ) {

        if( isset( $bean->contact_id_c ) ) {

            $bean->contact_id   = $bean->contact_id_c;
            $bean->contact_name = $bean->estudiante_c;

            /*$es = $bean->get_linked_beans('calls_contacts','Contacts');

            foreach( $es as $e )
            {
                if($e->id != $bean->contact_id_c)
                {
                    $bean->load_relationship('calls_contacts');
                    $bean->calls_contacts->delete($e->id);
                    $bean->calls_contacts->add($bean->contact_id_c);
                }
            }
            ...
            if(count($es) == 0)
            {
                ...
                //if($c->load_relationship('calls_contacts'))
                //    $c->calls_contacts->add($bean->contact_id_c);
            }*/
        }
